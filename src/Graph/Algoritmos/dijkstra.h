   ...
   public:
    ...
    unordered_map<Vertex<TV, TE>*, TE> apply(){
        ...
        while (!pq.empty()) 
        {
            ...
            // cout << " Nodo pop priroty-queque: " << u->data << endl;
            ...
            for (auto edg : u->edges) {
                ...
                // cout << "  A Nodo: " << v->data << " W actual: " <<  dist[u] + weight << endl;

                // si hay un camino más corto a v por u
                if (dist[v] > dist[u] + weight) {
                    // Updating distance of v 
                    dist[v] = dist[u] + weight; 
                    // cout << "    mas corto por : " << v->data << " W nuevo: " << dist[v] << endl;
                    pq.push(make_pair(dist[v], v));  

                    
                    /* Construyo el nuevo grafo resultante */
                        // cout << edg->vertexes[0]->data << " " << edg->vertexes[1]->data << endl;
                        // DGraph->display();
                        Vertex<TV, TE>* tp = DGraph->findByData(edg->vertexes[1]->data); // busco el nodo en el nuevo grafo
                        ...
                        if ( !tp->edges.empty() ) {
                            // cout << "del: " << tp->data << endl;
                            Edge<TV, TE>* tmp = tp->edges.back();
                            // cout << tmp->vertexes[0]->data << " " << tmp->vertexes[1]->data << endl;
                            // se elimina el edge que existe
                            DGraph->deleteEdge(DGraph->findByVertex(tmp->vertexes[0]), DGraph->findByVertex(tmp->vertexes[1]));
                        }
